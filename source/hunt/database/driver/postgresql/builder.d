module hunt.database.driver.postgresql.builder;
...
class PostgresqlSqlBuilder : SqlBuilder 
{
	...
	SqlBuilder where(string expression)
	{
		...
		if(arr.length != 3 && arr.length != 4){
			...
			if (arr.length == 3)
				expr = new WhereExpression(arr[0],arr[1],arr[2]);
			else 
				expr = new WhereExpression(arr[0],arr[1]~" "~arr[2], arr[3]);
			_whereKeys ~= expr;
			if(arr[$-1] == "?")_whereKeysParameters ~= expr;
		}
		return this;
	}

module hunt.database.driver.mysql.impl.codec.RowResultDecoder;
...
import hunt.database.driver.mysql.impl.codec.DataTypeCodec;
...
import hunt.database.driver.mysql.impl.MySQLRowImpl;
        ...
        if (rowDesc.dataFormat() == DataFormat.BINARY) {
            ...
            for (int c = 0; c < len; c++) {
                ...
                if (inBuffer.getUnsignedByte(inBuffer.readerIndex()) == NULL) {
                    ...
                    version(HUNT_DB_DEBUG_MORE) {
                        infof("    column[%d]: name=%s, type=%s, flags=%d, charset=%s", 
                            c, columnDesc.name(), dataType, columnDefinitionFlags, charset);
                    }

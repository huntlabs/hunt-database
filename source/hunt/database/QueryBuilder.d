...
module hunt.database.QueryBuilder;
...
class Expr
{
    public string eq(T)(string key,T value)
	{   
        static if (is(T == string))
		    return whereImpl(key,CompareType.eq,quoteSqlString(value));
        else
		    return whereImpl(key,CompareType.eq,value.to!string);
	}
    public string ne(T)(string key,T value)
	{
		static if (is(T == string))
		    return whereImpl(key,CompareType.ne,quoteSqlString(value));
        else
		    return whereImpl(key,CompareType.ne,value.to!string);
	}
    public string gt(T)(string key,T value)
	{
		static if (is(T == string))
		    return whereImpl(key,CompareType.gt,quoteSqlString(value));
        else
		    return whereImpl(key,CompareType.gt,value.to!string);
	}
    public string lt(T)(string key,T value)
	{
		static if (is(T == string))
		    return whereImpl(key,CompareType.lt,quoteSqlString(value));
        else
		    return whereImpl(key,CompareType.lt,value.to!string);
	}
    public string ge(T)(string key,T value)
	{
		static if (is(T == string))
		    return whereImpl(key,CompareType.ge,quoteSqlString(value));
        else
		    return whereImpl(key,CompareType.ge,value.to!string);
	}
    public string le(T)(string key,T value)
	{
		static if (is(T == string))
		    return whereImpl(key,CompareType.le,quoteSqlString(value));
        else
		    return whereImpl(key,CompareType.le,value.to!string);
	}
    public string like(T)(string key,T value)
	{
		static if (is(T == string))
		    return whereImpl(key,CompareType.like,quoteSqlString(value));
        else
		    return whereImpl(key,CompareType.like,value.to!string);
	}

    private string whereImpl(string key,CompareType type,string value)
	{
		return  (new WhereExpression(key,type,value)).toString();
	}
...
class QueryBuilder {
    ...
    public QueryBuilder join(JoinMethod joinMethod,string table,string tablealias,string joinWhere)
	{
    ...
    public static string parameterized(string sql , Object[string] params)
    {
        foreach(k , v ; params) {
            if(cast(String)v !is null)
            {
                sql = sql.replace(":"~k, quoteSqlString(v.toString()));
            }
            else
            {
                sql = sql.replace(":"~k, v.toString());
            }
        }
        return sql;
    }
		...
		switch(_type){
			...
			case QUERY_TYPE.INSERT:
				{
                    ...
                    foreach(k,v; _values){
                        ...
                        {
                            values ~= quoteSqlString(v.value.toString()) ~ ",";
                        }
                        else
			...
			case QUERY_TYPE.DESC_TABLE:
                if(_dbType == DBType.POSTGRESQL.name)
                    ...
                    str ~= "select * from sqlite_master where type=\"table\" and name=\""~_table~"\"";
				break;
			default:
